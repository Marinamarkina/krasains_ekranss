...
namespace Viktorina
{
    public partial class Form2 : Form
    {
        ...
        int jautajumsNr = 0;
        List<int> jautajumaIndexi = new List<int>();
        int momentaIndex = 0;
        ...
        void Parbaude(int atbild)
         {
            string[] poga = new string[3] { butAtbilde1.Text, butAtbilde2.Text, butAtbilde3.Text };

            if (poga[atbild] == atbildes[momentaIndex])
            {
                punkti += 1;
            }
            else if (punkti > 0)
            ...
            if (punkti == 1)
            {
                labPunkti.Text = punkti + " punkts";
            }
            else
            {
                labPunkti.Text = punkti + " punkti";
            }
            ...
            {
                JautajumuGenerators();
                labPiemers.Text = (jautajumsNr + 1) + ". no 10 jautājumiem";
        ...
        public void JautajumuGenerators ( )
        {

            momentaIndex = jautajumaIndexi[jautajumsNr];
          
            labJautajums.Text = jautajumi[momentaIndex];
            butAtbilde1.Text = atbildes1[momentaIndex];
            butAtbilde2.Text = atbildes2[momentaIndex];
            butAtbilde3.Text = atbildes3[momentaIndex];
            
        }
       
        private void JautajumuMaisitajs()
        {
            for (int i = 0; i < 20; i++)
            {
                jautajumaIndexi.Add(i);
            }
            Random rng = new Random();
            jautajumaIndexi = jautajumaIndexi.OrderBy(a => rng.Next()).ToList();
        }
